...
var isVisible = (element) => {
  if (false)
    return true;
  const computed5 = getComputedStyle(element);
  return computed5.position === "fixed" ? false : element.offsetParent !== null;
};
...
import { shallowRef as shallowRef2, watchEffect as watchEffect2, readonly, unref as unref2, ref as ref2, isVue3, version, watch as watch2, customRef, getCurrentScope, onScopeDispose as onScopeDispose2, effectScope, provide as provide2, inject as inject2, isRef as isRef2, computed as computed3, reactive, toRefs as toRefs$1, toRef, isVue2, set as set$1, getCurrentInstance as getCurrentInstance2, onBeforeMount as onBeforeMount2, nextTick as nextTick2, onBeforeUnmount as onBeforeUnmount2, onMounted as onMounted2, onUnmounted as onUnmounted2, isReactive } from "vue-demi";
...
function computedWithControl(source, fn2) {
  ...
  let track;
  let trigger;
  const dirty = ref2(true);
  const update2 = () => {
    dirty.value = true;
    trigger();
  };
  watch2(source, update2, { flush: "sync" });
...
function refDebounced(value, ms = 200, options = {}) {
  const debounced = ref2(value.value);
...
import { isRef as isRef3, ref as ref3, shallowRef as shallowRef3, watchEffect as watchEffect3, computed as computed4, inject as inject3, unref as unref3, watch as watch3, getCurrentInstance as getCurrentInstance3, customRef as customRef2, onUpdated, reactive as reactive2, nextTick as nextTick3, onMounted as onMounted3, markRaw, readonly as readonly2, getCurrentScope as getCurrentScope2, isVue2 as isVue22, set, del, isReadonly, onBeforeUpdate } from "vue-demi";
...
function useEventListener(...args) {
  ...
  const stopWatch = watch3(() => [unrefElement(target2), resolveUnref(options)], ([el, options2]) => {
    cleanup();
    if (!el)
      return;
...
function useDocumentVisibility({ document: document2 = defaultDocument } = {}) {
  if (!document2)
    return ref3("visible");
  const visibility = ref3(document2.visibilityState);
...
function useResizeObserver(target2, callback, options = {}) {
  ...
  const stopWatch = watch3(() => unrefElement(target2), (el) => {
    cleanup();
...
function useElementBounding(target2, options = {}) {
  ...
  watch3(() => unrefElement(target2), (ele) => !ele && update2());
  if (windowScroll)
...
function useVModel(props, key, emit, options = {}) {
  ...
  if (passive) {
    ...
    watch3(() => props[key], (v) => proxy.value = cloneFn(v));
    watch3(proxy, (v) => {
      if (v !== props[key] || deep)
        _emit(event, v);
    }, { deep });
...
function useWindowFocus({ window: window2 = defaultWindow } = {}) {
  if (!window2)
    return ref3(false);
  const focused = ref3(window2.document.hasFocus());
...
function baseDifference(array, values2, iteratee2, comparator) {
  ...
  outer:
    while (++index < length) {
      var value = array[index], computed5 = iteratee2 == null ? value : iteratee2(value);
      value = comparator || value !== 0 ? value : 0;
      if (isCommon && computed5 === computed5) {
        var valuesIndex = valuesLength;
        while (valuesIndex--) {
          if (values2[valuesIndex] === computed5) {
            continue outer;
          }
        }
        result2.push(value);
      } else if (!includes2(values2, computed5, comparator)) {
        result2.push(value);
      }
    }
  return result2;
...
function baseIntersection(arrays, iteratee2, comparator) {
  ...
  outer:
    while (++index < length && result2.length < maxLength) {
      var value = array[index], computed5 = iteratee2 ? iteratee2(value) : value;
      value = comparator || value !== 0 ? value : 0;
      if (!(seen ? cacheHas_default(seen, computed5) : includes2(result2, computed5, comparator))) {
        othIndex = othLength;
        while (--othIndex) {
          var cache = caches[othIndex];
          if (!(cache ? cacheHas_default(cache, computed5) : includes2(arrays[othIndex], computed5, comparator))) {
            continue outer;
          }
        }
        if (seen) {
...
function baseExtremum(array, iteratee2, comparator) {
  ...
  while (++index < length) {
    ...
    if (current != null && (computed5 === void 0 ? current === current && !isSymbol_default(current) : comparator(current, computed5))) {
      var computed5 = current, result2 = value;
    }
  }
  return result2;
}
...
function basePullAll(array, values2, iteratee2, comparator) {
  ...
  while (++index < length) {
    var fromIndex = 0, value = values2[index], computed5 = iteratee2 ? iteratee2(value) : value;
    while ((fromIndex = indexOf2(seen, computed5, fromIndex, comparator)) > -1) {
      if (seen !== array) {
...
function baseSortedIndexBy(array, value, iteratee2, retHighest) {
  ...
  while (low < high) {
    var mid = nativeFloor4((low + high) / 2), computed5 = iteratee2(array[mid]), othIsDefined = computed5 !== void 0, othIsNull = computed5 === null, othIsReflexive = computed5 === computed5, othIsSymbol = isSymbol_default(computed5);
    if (valIsNaN) {
      ...
      setLow = retHighest ? computed5 <= value : computed5 < value;
    }
    if (setLow) {
      low = mid + 1;
    } else {
...
function baseSortedIndex(array, value, retHighest) {
  ...
  if (typeof value == "number" && value === value && high <= HALF_MAX_ARRAY_LENGTH) {
    while (low < high) {
      var mid = low + high >>> 1, computed5 = array[mid];
      if (computed5 !== null && !isSymbol_default(computed5) && (retHighest ? computed5 <= value : computed5 < value)) {
        low = mid + 1;
      } else {
        high = mid;
      }
...
function baseSortedUniq(array, iteratee2) {
  ...
  while (++index < length) {
    var value = array[index], computed5 = iteratee2 ? iteratee2(value) : value;
    if (!index || !eq_default(computed5, seen)) {
      var seen = computed5;
  ...
  outer:
    while (++index < length) {
      ...
      if (isCommon && computed5 === computed5) {
        ...
        result2.push(value);
      } else if (!includes2(seen, computed5, comparator)) {
        if (seen !== result2) {
          seen.push(computed5);
        }
        result2.push(value);
  ...
  outer:
    while (length-- && resIndex < takeCount) {
      ...
      while (++iterIndex < iterLength) {
        var data = iteratees[iterIndex], iteratee2 = data.iteratee, type = data.type, computed5 = iteratee2(value);
...
var getStyle = (element, styleName) => {
  ...
  try {
    ...
    if (style)
      return style;
    const computed5 = (_a2 = document.defaultView) == null ? void 0 : _a2.getComputedStyle(element, "");
    return computed5 ? computed5[key] : "";
  } catch (e) {
...
var useDeprecated = ({ from, replacement, scope, version: version2, ref: ref4, type = "API" }, condition) => {
  watch(() => unref(condition), (val) => {
    if (val) {
      debugWarn(scope, `[${type}] ${from} is about to be deprecated in version ${version2}, please use ${replacement} instead.
For more detail, please visit: ${ref4}
`);
    }
  }, {
    immediate: true
...
function Zt(t) {
  var e = t.state, n = t.name, r = e.rects.reference, o = e.rects.popper, i = e.modifiersData.preventOverflow, a = ne(e, { elementContext: "reference" }), s = ne(e, { altBoundary: true }), f = gt2(a, r), c = gt2(s, o, i), u = yt(f), m = yt(c);
...
function getScale(element) {
  ...
  let x = (fallback ? round2(rect.width) : rect.width) / width;
...
function autoUpdate(reference, floating, update2, options) {
  ...
  const ancestors = ancestorScroll || ancestorResize ? [...isElement3(reference) ? getOverflowAncestors(reference) : reference.contextElement ? getOverflowAncestors(reference.contextElement) : [], ...getOverflowAncestors(floating)] : [];
  ...
  if (elementResize) {
    ...
    isElement3(reference) && !animationFrame && observer.observe(reference);
    if (!isElement3(reference) && reference.contextElement && !animationFrame) {
...
lodash-es/lodash.js:
   ...
   *)
*/
//# sourceMappingURL=chunk-NO7HEG3Z.js.map
